...
namespace sl {
    ...
    class ImageHandler {
        ...
        public void initialize() {
            ...
            [18] { -1.0f, -1.0f, 0.0f, 1.0f, -1.0f, 0.0f, -1.0f, 1.0f, 0.0f, -1.0f, 1.0f, 0.0f, 1.0f, -1.0f, 0.0f, 1.0f, 1, 0.0f };

            quad_vb = Gl.GenBuffer();
            Gl.BindBuffer(BufferTarget.ArrayBuffer, quad_vb);
            Gl.BufferData(
                BufferTarget.ArrayBuffer,
                (uint)(sizeof(float) * g_quad_vertex_buffer_data.Length),
    ...
    class PointCloud {
        ...
        public void initialize(Resolution res) {
            ...
            Gl.BufferData(
                BufferTarget.ArrayBuffer,
                (uint)res.height * (uint)res.width * sizeof(float) * 4,
                IntPtr.Zero,
                BufferUsage.StaticDraw
            );
            Gl.BindBuffer(BufferTarget.ArrayBuffer, 0);
            ...
            // mat_.Create(new Resolution(Math.Min((uint)res.width, 720), Math.Min((uint)res.height, 404)), sl.MAT_TYPE.MAT_32F_C4,
            // MEM.CPU);
        }
        ...
        public void draw(Matrix4x4 vp) {
            if (mat_.IsInit()) {
                ...
                Gl.BufferData(
                    BufferTarget.ArrayBuffer,
                    (uint)mat_.GetResolution().height * (uint)mat_.GetResolution().width * sizeof(float) * 4,
                    mat_.GetPtr(),
                    BufferUsage.StaticDraw
                );
        ...
        public void addCylinder(float3 startPosition, float3 endPosition, float4 clr, float radius = 0.010f) {
            ...
            for (int j = 0; j < NB_SEG; j++) {
                ...
                v2 = rotationMatrix.multiply(new float3(m_radius * (float)Math.Cos(i), m_height, m_radius * (float)Math.Sin(i)))
                         .add(startPosition);
                v4 = rotationMatrix.multiply(new float3(m_radius * (float)Math.Cos(i1), m_height, m_radius * (float)Math.Sin(i1)))
                         .add(startPosition);
                v3 = rotationMatrix.multiply(new float3(m_radius * (float)Math.Cos(i1), 0, m_radius * (float)Math.Sin(i1)))
                         .add(startPosition);
