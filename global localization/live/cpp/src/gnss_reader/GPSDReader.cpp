...
std::shared_ptr<IGNSSReader> IGNSSReader::create(sl::Camera* zed, bool read_from_svo) {
    ...
    if (read_from_svo) {
        ...
        reader->initialize(zed, read_from_svo);
    }
#ifdef USE_GPSD
    else {
        reader = std::make_shared<GPSDReader>();
...
#endif
    // Add other GNSS reader types here as needed
    return reader;
}
...
sl::GNSSData SVOReader::getNextGNSSValue(uint64_t current_timestamp) {
