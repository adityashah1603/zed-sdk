...
# Audio file paths - update these to point to your audio files
AUDIO_LEFT_PATH = os.path.join(BASE_DIR, "audio_left.wav")  # Change to your left audio file
AUDIO_RIGHT_PATH = os.path.join(BASE_DIR, "audio_right.wav")  # Change to your right audio file
...
def play_audio(direction: str) -> bool:
    """
    Play audio file based on direction (LEFT or RIGHT).
    Returns True if audio was played successfully, False otherwise.
    """
    audio_path = AUDIO_LEFT_PATH if direction.upper() == "LEFT" else AUDIO_RIGHT_PATH
    ...
    try:
        ...
        print(f"[AUDIO] Playing {direction} audio: {os.path.basename(audio_path)}")
        return True
...
def main() -> int:
    ...
    if not os.path.isfile(AUDIO_LEFT_PATH):
        print(f"[WARN] Left audio file not found: {AUDIO_LEFT_PATH}")
        print("[WARN] Audio alerts will be disabled for LEFT direction.")
    if not os.path.isfile(AUDIO_RIGHT_PATH):
        print(f"[WARN] Right audio file not found: {AUDIO_RIGHT_PATH}")
