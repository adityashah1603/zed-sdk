...
namespace sl {
    ...
    /// This class also handles pose of the camera to be able to retrieve the pose of the camera at the object timestamp (obviously in the
    /// past). If WITH_IMAGE_RETENTION flag is activated, the images/depth/point cloud will be stored via the push() and pop() synchonized
    /// to the datas. \warning Note that image retention consumes a lot of CPU and GPU memory since we need to store them in memory so that
    /// we can output them later. As an example, for a latency of 2s, it consumes between 500Mb and 1Gb of memory for CPU and same for GPU.
    /// Make sure you have enought space left on memory
