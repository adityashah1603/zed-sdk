...
void TrackingViewer::render_2D(
    cv::Mat& left_display,
    sl::float2 img_scale,
    std::vector<sl::ObjectData>& objects,
    bool render_mask,
    bool isTrackingON
) {
    ...
    for (auto& obj : objects) {
        if (renderObject(obj, isTrackingON)) {
            ...
            putText(
                left_display,
                toString(obj.label).get(),
                cv::Point2d(position_image.x - 20, position_image.y - 12),
            ...
            putText(
                ...
                cv::Point2d(position_image.x - 20, position_image.y - 30),
                cv::FONT_HERSHEY_COMPLEX_SMALL,
                0.5,
                cv::Scalar(255, 255, 255, 255),
                1
            ...
            if (std::isfinite(obj.position.z)) {
                ...
                putText(
                    left_display,
                    text,
                    cv::Point2d(position_image.x - 20, position_image.y),
                    cv::FONT_HERSHEY_COMPLEX_SMALL,
                    0.5,
                    cv::Scalar(255, 255, 255, 255),
                    1
...
void TrackingViewer::generate_view(
    sl::Objects& objects,
    cv::Mat& image_left_ocv,
    sl::float2 img_scale,
    sl::Pose current_camera_pose,
    cv::Mat& tracking_view,
...
void TrackingViewer::pruneOldPoints(uint64_t ts) {
    ...
    for (size_t track_index = 0; track_index < tracklets.size(); ++track_index) {
        if (tracklets[track_index].is_alive) {
            ...
            while (tracklets[track_index].positions_to_draw.size() > 0
                   && tracklets[track_index].positions_to_draw.front().timestamp < ts - history_duration) {
                ...
                case sl::OBJECT_CLASS::VEHICLE:
                    {
                        ...
                        cv::rectangle(
                            ...
                            clr,
                            -1
                        ); // OpenCV 2.X --> cv::FILLED not defined but negative values
#else
...
void TrackingViewer::drawCamera() {
    ...
    uchar clr[4]
        = {static_cast<uchar>(camera_color(0)),
           static_cast<uchar>(camera_color(1)),
           static_cast<uchar>(camera_color(2)),
           static_cast<uchar>(camera_color(3))};
    // Draw FOV
...
void TrackingViewer::drawHotkeys() {
    ...
    cv::putText(background, "Press 'i' to zoom in", cv::Point2i(25, window_height - 25), 1, 1.0, hotkeys_clr, 1);
    cv::putText(background, "Press 'o' to zoom out", cv::Point2i(25, window_height - 15), 1, 1.0, hotkeys_clr, 1);
}
...
cv::Point2i TrackingViewer::toCVPoint(sl::float3 position, sl::Pose pose) {
    ...
    return cv::Point2i(
        static_cast<int>((new_position.tx - x_min) / x_step + .5f),
        static_cast<int>((new_position.tz - z_min) / z_step + .5f)
    );
}
