...
""")
    ...
    while viewer.is_available() and not exit_signal:
        if zed.read() <= sl.ERROR_CODE.SUCCESS: # Get a new image
            ...
            # -- Get the image, always on the GPU to showcase the use of sl.Mat.convert_color
            lock.acquire()
            zed.retrieve_image(image_left_tmp, sl.VIEW.LEFT, sl.MEM.GPU, resolution=display_resolution)
