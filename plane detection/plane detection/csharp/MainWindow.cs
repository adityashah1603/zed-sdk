        ...
        private void NativeWindow_MouseDown(object sender, NativeWindowMouseEventArgs e) {
            if (e.Buttons == MouseButton.Left) {
                ...
                Vector2 screenPos = new Vector2(
                    (float)e.Location.X / (float)zedCamera.ImageWidth,
                    ((float)zedCamera.ImageHeight - (float)e.Location.Y) / (float)zedCamera.ImageHeight
                );
            ...
            if (viewer.isAvailable() && zedCamera.Grab(ref runtimeParameters) <= ERROR_CODE.SUCCESS) {
                if (zedMat.IsInit()) {
                    ...
                    if (tracking_state == POSITIONAL_TRACKING_STATE.OK) {
                        timer++;
                        if (userAction.hit) {
                            Vector2 imageClick = new Vector2(
                                (float)userAction.hitCoord.X * (float)zedCamera.ImageWidth,
